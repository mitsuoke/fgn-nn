...
| `commercial-pages.js` | Поведение FAQ на коммерческих страницах (раскрыт только один ответ за раз); оставлен отдельным файлом для явной изоляции компонента |
...
CRM-форма зависит от доступности загрузчика Bitrix24. Если внешний скрипт не загрузился, необходимо проверить сеть, CSP и доступность CDN, не отключая защитные ограничения на опубликованном сайте. CSP коммерческих страниц разрешает только `cdn-ru.bitrix24.ru`, `b24-ud1314.bitrix24.ru` и хэш точного inline-кода. Официальная форма добавляет стили динамически, поэтому `'unsafe-inline'` обоснованно разрешён только в `style-src`; добавлять его в `script-src` запрещено.
...
- точные CSP-хэши embed-кодов форм №10 и №16 вместо разрешения произвольного inline-JavaScript;
- `Referrer-Policy`;
- `object-src 'none'`;
...
Если CRM-форма не отображается, сначала проверить доступность `cdn-ru.bitrix24.ru`, затем CSP и консоль браузера. После изменения embed-кода нужно пересчитать его SHA-256-хэш, иначе браузер закономерно заблокирует скрипт. В основном блоке резервными каналами остаются телефон и MAX; e-mail доступен в подвале.
