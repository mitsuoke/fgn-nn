...
- Публичный сайт: <https://fgn-nn.ru/>
...
- предупреждение «БАД. Не является лекарственным средством» используется только для ежовика; для остальных десяти товаров применяется «Пищевая добавка. Не является лекарственным средством.».
...
CRM-формы Bitrix24 №8, №10 и №16 загружаются не напрямую в основные страницы, а через локальный служебный iframe `forms/bitrix.html`. Конфигурация публичных loader-кодов находится в `forms/bitrix-embed.js`. Требуемый Bitrix24 `unsafe-eval` ограничен CSP служебной страницы формы и отсутствует в CSP родительских страниц. Иframe остаётся same-origin, поэтому это CSP-изоляция, а не полноценный security sandbox. Секретные вебхуки и API-токены в репозитории не используются.
...
| `start.html` | Страница «Запустить продукт» для посетителя, которому нужна помощь от идеи до партии; использует общую CRM-форму №16 |
...
| `fasovka-chaya-i-sborov/index.html` | SEO-страница фасовки чая и травяных сборов на заказ |
| `upakovka-i-markirovka-bad/index.html` | SEO-страница упаковки и маркировки БАД на заказ |
...
| `script.js` | Навигация, анимации, Метрика, согласие на аналитику, цели и текущий год |
| `commercial-pages.js` | Поведение FAQ на коммерческих страницах; общий обработчик изменения высоты CRM-iframe находится в `script.js` |
| `data/products.json` | Единый оперативный источник данных 11 товаров |
| `shop.js` | Гидратация главной карусели, каталога, карточек товара и галерей с сохранением HTML-резерва |
| `products-catalog.css` | Единый визуальный шаблон каталога |
...
| `scripts/browser-smoke.mjs` | Playwright smoke-тест маршрутов, UI, изображений, мобильной адаптации и CRM-iframe с mock Bitrix |
| `scripts/production-bitrix-smoke.mjs` | Read-only проверка реальных Bitrix-форм на production; форму не отправляет |
...
| `.github/workflows/bitrix-production-smoke.yml` | Ежедневная и ручная проверка реальных CRM-форм на production |
...
| --- | --- | --- |
| Главная | `/` | Представить производство и привести к обращению |
| Запустить продукт | `/start.html` | Объяснить запуск продукта новому или действующему бренду и привести к заявке через CRM-форму №16 |
| Капсулирование | `/kapsulirovanie/` | Получить запрос на капсулирование от 100 банок |
| Фасовка сухой продукции | `/fasovka-sypuchih-produktov/` | Получить запрос на фасовку смесей, порошков, чая и сырья |
| Фасовка чая и сборов | `/fasovka-chaya-i-sborov/` | Получить запрос на фасовку чая и травяных сборов |
| Упаковка и маркировка БАД | `/upakovka-i-markirovka-bad/` | Получить запрос на упаковку и маркировку готовой продукции |
| Контрактное производство БАД | `/kontraktnoe-proizvodstvo-bad/` | Получить запрос на полный цикл производства |
| Политика | `/privacy.html` | Описать порядок обработки данных и аналитику |
| Согласие | `/consent.html` | Зафиксировать условия согласия отправителя обращения |
...
9. автоматический вывод текущего года.
...
Файл отвечает за работу раскрывающихся FAQ на коммерческих страницах по принципу «открыт один ответ»: при раскрытии нового `<details class="faq-item">` закрывает остальные элементы того же `.faq-list`. Логика вынесена в отдельный файл и подключается только на страницах, где есть FAQ.
...
На сайте используются три официальные inline-формы Bitrix24:

- главная — форма №8 (`data-b24-form="inline/8/kodg8f"`);
- `/kapsulirovanie/` — форма №10 (`data-b24-form="inline/10/20s329"`);
- `start.html` и остальные четыре коммерческие страницы — одна общая форма №16 (`data-b24-form="inline/16/5s6fmf"`).

Разметка, поля, валидация, согласие и отправка управляются настройками форм в Bitrix24. Публичные загрузчики находятся на `cdn-ru.bitrix24.ru`; секретные вебхуки и API-токены не используются. Заголовок и пояснение коммерческого блока находятся в HTML сайта, а заголовки самих форм в Bitrix24 оставлены пустыми.

На коммерческих страницах общий контейнер `.commercial-crm-panel`, ссылка `.commercial-crm-consent` и альтернативные действия `.service-contact-options` оформлены централизованно в `commercial-pages.css`. Основные CTA ведут к `#contact` той же страницы. Возле каждой формы есть явная ссылка на `https://fgn-nn.ru/consent.html`. Ссылка e-mail удалена из блока расчёта, но адрес остаётся в подвале как резервный контакт. На `start.html` форма №16 встроена отдельно в блок `#request` через тот же `forms/bitrix.html`; альтернативными каналами там остаются MAX, телефон и e-mail.

Перед production-публикацией настройки каждой формы в Bitrix24 проверяются отдельно: согласие не должно быть отмечено заранее, в полном тексте должны быть указаны ООО «Бурундук Шоп», юридический адрес и актуальный адрес для отзыва, а ссылка на отдельное согласие должна вести на действующий документ сайта. Эти параметры приходят из Bitrix24 и не исправляются HTML-кодом сайта.

В конфигурации официального виджета Bitrix24 подтверждены события успешной отправки `B24_FORM_10_END` и `B24_FORM_16_END`. Соответствующие цели Метрики создаются только после контрольной отправки на production. Содержимое полей формы нельзя передавать в аналитику.

### Известное ограничение

CRM-форма зависит от доступности Bitrix24. Если она не отображается, нужно проверить загрузку `forms/bitrix.html`, затем `forms/bitrix-embed.js`, CDN Bitrix24 и сообщения CSP. Нельзя переносить `unsafe-eval` из CSP служебной страницы формы в CSP основной страницы. Также нельзя возвращать прямой `<script data-b24-form>` на главную, `start.html` или коммерческие страницы.

### Если форма будет заменена собственной интеграцией
...
- не помещать в клиентский код секреты, вебхуки и API-токены Bitrix24; публичный код загрузчика текущей CRM-формы секретом не является;
...
При переходе на собственную API-интеграцию необходимо:

1. отправлять обращения в Bitrix24 через защищённый серверный обработчик;
2. показывать понятное состояние загрузки;
3. блокировать повторную отправку во время запроса;
4. вызывать аналитическую цель только после подтверждённой успешной отправки и не передавать персональные данные;
5. обновить `privacy.html`, `consent.html` и настройки согласия до запуска;
6. проверить российское место первичного хранения заявок;
7. протестировать попадание всех полей в нужную воронку Bitrix24 и проверить права доступа сотрудников.
...
| `email_click` | переход по резервной ссылке `mailto:` в подвале |
| `start_product_click` | переход на `start.html` |
| `B24_FORM_10_END` | успешная отправка формы капсулирования №10; создать цель после production-теста |
| `B24_FORM_16_END` | успешная отправка общей короткой формы №16; создать цель после production-теста |
...
При изменении состава полей, способа отправки, получателей или CRM необходимо до публикации проверить `privacy.html`, `consent.html` и настройки согласия внутри CRM-формы Bitrix24.
...
- изолированный CRM-shell для форм №8, №10 и №16;
- `unsafe-eval` ограничен только служебным документом CRM-формы;
- `Referrer-Policy`;
- `object-src 'none'`;
...
http://localhost:8000/fasovka-chaya-i-sborov/
http://localhost:8000/upakovka-i-markirovka-bad/
...
- резервный e-mail в подвале открывает почтовую программу;
- CRM-форма требует необходимые контактные данные и согласие;
- после успешной отправки создаются клиент и сделка в Bitrix24, а пользователь получает понятное сообщение об успешной отправке;
...
После изменения CSS или JavaScript увеличить номер версии во всех HTML-файлах, которые подключают изменённый файл. Для общего `styles.css` версия должна быть одинаковой на всём сайте. Иначе часть посетителей может получить новый HTML со старым кэшированным кодом.
...
- при необходимости добавить цель `lead_sent` только после подтверждённой успешной отправки CRM-формы; персональные данные в Метрику не передавать;
...
Если CRM-форма не отображается, сначала проверить доступность `cdn-ru.bitrix24.ru`, затем CSP и консоль браузера. После изменения формы нужно обновить конфигурацию в `forms/bitrix-embed.js` и проверить `npm run check`, `npm run test:smoke` и read-only production smoke. В основном блоке резервными каналами остаются телефон и MAX; e-mail доступен в подвале.
...
| Заявки CRM | встроенная форма Bitrix24 → «Сделка + клиент» → воронка «Заявки с сайта FGN» |
