...
На главной странице встроена официальная inline-форма Bitrix24 (`data-b24-form="inline/8/kodg8f"`). Её разметка, поля, валидация, согласие и отправка управляются настройками формы в Bitrix24, а сайт загружает публичный скрипт с `cdn-ru.bitrix24.ru`.

Отправка встроенной CRM-формы Bitrix24 сейчас не регистрируется отдельной целью Яндекс Метрики. Содержимое полей формы нельзя передавать в аналитику.

### Известное ограничение

CRM-форма зависит от доступности загрузчика Bitrix24. Если внешний скрипт Bitrix24 не загрузился, необходимо проверить сеть, Content Security Policy и доступность CDN, не отключая защитные ограничения на опубликованном сайте.
...
| — | отдельная цель отправки CRM-формы Bitrix24 сейчас не используется |
...
Если CRM-форма не отображается, сначала проверить доступность `cdn-ru.bitrix24.ru`, затем Content Security Policy и консоль браузера. В качестве резервных каналов связи на сайте остаются телефон, MAX и e-mail.
