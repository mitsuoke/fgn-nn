...
На коммерческих страницах общий контейнер `.commercial-crm-panel` и альтернативные действия `.service-contact-options` оформлены централизованно в `commercial-pages.css`. Основные CTA ведут к `#contact` той же страницы. Ссылка e-mail удалена из блока расчёта, но адрес остаётся в подвале как резервный контакт.
