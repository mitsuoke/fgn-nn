...
| Запустить продукт | `/start.html` | Объяснить запуск продукта новому или действующему бренду |
...
- остальные четыре коммерческие страницы — одна общая форма №16 (`data-b24-form="inline/16/5s6fmf"`).
...
На коммерческих страницах общий контейнер `.commercial-crm-panel`, ссылка `.commercial-crm-consent` и альтернативные действия `.service-contact-options` оформлены централизованно в `commercial-pages.css`. Основные CTA ведут к `#contact` той же страницы. Возле каждой формы есть явная ссылка на `https://fgn-nn.ru/consent.html`. Ссылка e-mail удалена из блока расчёта, но адрес остаётся в подвале как резервный контакт.
...
CRM-форма зависит от доступности Bitrix24. Если она не отображается, нужно проверить загрузку `forms/bitrix.html`, затем `forms/bitrix-embed.js`, CDN Bitrix24 и сообщения CSP. Нельзя переносить `unsafe-eval` из CSP служебной страницы формы в CSP основной страницы. Также нельзя возвращать прямой `<script data-b24-form>` на главную или коммерческие страницы.
